...
- **验证请求**：API Server 首先验证请求的合法性（语法检查，用户权限）	
	- **存储到 etcd**：验证通过后，API Server 将 Pod 的配置信息（`spec`）存储到 **etcd** 中，此时 Pod 状态为 `Pending`（未调度），`status` 字段为空。
	- **返回确认**：API Server 向用户返回 “创建成功” 的响应（此时 Pod 仅在 etcd 中存在记录，尚未实际运行）。
3. **调度器（kube-scheduler）绑定节点**
   - **监听待调度 Pod**：调度器通过监听 API Server 中状态为 `Pending` 且未分配节点（`spec.nodeName` 为空）的 Pod。
   ...
   - **绑定节点**：调度器向 API Server 发送 **绑定请求**，将 Pod 的 `spec.nodeName` 字段更新为选中的节点名称，API Server 再更新 etcd 中Pod的节点绑定信息。
4. **kubelet 发现并创建 Pod**
   - **监听 Pod**：每个节点上的 **kubelet** 会定期向 
   - Server 查询 “分配给本节点的 Pod”（通过 `spec.nodeName` 匹配自身节点名）。
